...
import { getRoutine, getSession, getSessionForEvent } from '@/lib/routines';
...
export default function RoutineSessionPage() {
    ...
    async function load() {
      try {
        const [r, s] = await Promise.all([
          getRoutine(id),
          calendarEventId
            ? getSessionForEvent(id, calendarEventId)
            : getSession(id, TODAY),
        ]);
        setRoutine(r);
        setSession(s);
  ...
  return (
    <div className="animate-fade-in">
      ...
      <SessionView routine={routine} initialSession={session} calendarEventId={calendarEventId} />
    </div>
  );
}
